...
protected:
    std::unique_ptr<Pizza> createPizza(const std::string &type) override {
        ...
        if (type == "cheese") {
            pizza =
                std::make_unique<CheesePizza>(std::make_unique<NewYorkPizzaIngredientFactory>());
            pizza->setName("New York Style Cheese Pizza");
        } else if (type == "pepperoni") {
            pizza =
                std::make_unique<PepperoniPizza>(std::make_unique<NewYorkPizzaIngredientFactory>());
            ...
            pizza =
                std::make_unique<VeggiePizza>(std::make_unique<NewYorkPizzaIngredientFactory>());
